package com.github.malyszaryczlowiek.cpcdb.properties;
...
public class SecureProperties
{
    ...
    private static final Object lock = new Object(); //TODO to można zamienić na SecureProperties.class
    ...
    public static void loadProperties()
    {
        ...
        if ( propertiesFile.exists() ) threadLoadingProperties.start();
    ...
    public static void saveProperties() {
        if ( propertiesFile.delete() ) System.out.println("Properties file has been deleted.");
        else System.out.println("Cannot delete properties file. Probably there is no access to file."); // te dwie linijki można zamienić na log
        if (checkInvalidityOfKey()) generateNewKey();
        try ( FileOutputStream fileOutputStream = new FileOutputStream("propertiesFile") ) {
            mapOfDecryptedProperties.forEach( (stringKey, stringValue) -> {
                ...
                try {
                    ...
                    byte[] binaryKeyLength = ByteBuffer.allocate( Integer.BYTES ).putInt( keyLength ).array();
                    byte[] binaryValueLength = ByteBuffer.allocate( Integer.BYTES ).putInt( valueLength ).array();
    ...
    public static void setProperty(String propertyName, String propertyValue) {
        if ( mapOfDecryptedProperties.containsKey(propertyName) ) mapOfDecryptedProperties.replace(propertyName, propertyValue);
        else mapOfDecryptedProperties.put(propertyName, propertyValue);
    }
    ...
    public static String getProperty(String propertyName) {
        if ( mapOfDecryptedProperties.size() > 0) return mapOfDecryptedProperties.get(propertyName);
        else return "";
    }

    public static boolean hasProperty(String property) { return mapOfDecryptedProperties.containsKey(property); }

    private static byte[] encryptString(String string) {
        try { return cipherEncryption.doFinal( string.getBytes( StandardCharsets.UTF_8 ) ); }
    ...
    private static byte[] decryptString( byte[] encryptedString ) {
        try { return cipherDecryption.doFinal( encryptedString ); } // returns decrypted text
        catch ( IllegalBlockSizeException | BadPaddingException e) {
            e.printStackTrace();
            return new byte[0];
        }
    ...
    private static void refactorLoadedBytesToMap() {
        ...
        while (index < loadedByteProperties.length) {
            ...
            if ( index == loadedByteProperties.length ) break;
        }
    }
    ...
    private static boolean checkInvalidityOfKey() {
        if ( "always".equals(getProperty("settings.keyValidityDuration") ))  return false;
        else {
            LocalDate today = LocalDate.now();
    ...
    private static void generateNewKey() {
        try {
            ...
            KeyStore.SecretKeyEntry secretKeyEntry = new KeyStore.SecretKeyEntry( secretKey );
            KeyStore.ProtectionParameter protectionParameter =
                    new KeyStore.PasswordProtection( "some7Simple5Passphrase3To2Get1PropertiesKey".toCharArray() );
...
// old class with live deciphering
...
/*
    public static boolean hasProperty(String property) {  return mapOfProperties.containsKey(property); }
 */
